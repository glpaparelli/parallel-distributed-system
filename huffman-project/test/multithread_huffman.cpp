...
int main() {
    ...
    vector<int> char_frequency = frequency_counters::multithread_counter(file_content, num_thread);
    ...
    start = std::chrono::high_resolution_clock::now();
    string file_content_compressed = ascii_encodings::multithread_binary_to_ASCII(file_content_encoded, num_thread);
    end = std::chrono::high_resolution_clock::now();
    elapsed = end - start;
